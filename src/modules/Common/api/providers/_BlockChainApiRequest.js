...
const RestRequest = axios.create({
	baseURL: process.env.VUE_APP_BLOCK_CHAIN_HOST_URL + process.env.VUE_APP_BLOCK_CHAIN_API_BASE_PATH,
	timeout: 150000, // 15s
...
RestRequest.interceptors.request.use(config => {
	return new Promise((resolve, reject) => {
		config.headers.Authorization = store.state.Common.accessToken || process.env.VUE_APP_WEB_ADMIN_ACCESS_TOKEN
		resolve(config)
	})
}, error => {
