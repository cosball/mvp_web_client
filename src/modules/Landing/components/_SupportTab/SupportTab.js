...
export default {
	...
	computed: {
		isComplete() {
			let isValid = true
			Object.keys(this.inputs.support).forEach((key, value) => {
				if (!this.inputs.support[key]) {
					isValid = false
					return false
				}
			})
	...
	methods: {
			...
			return new Promise((resolve, reject) => {
				this.S3Client.uploadFile(fileObject).then((data) => {
					return this.$store.dispatch('Landing/CONTACT_US', {
						'fullname': this.inputs.support.full_name,
						'email': this.inputs.support.email,
						'category': 'support',
						'message': this.inputs.support.message,
		...
		resetForm() {
			Object.keys(this.inputs.support).forEach((key) => {
				this.inputs.support[key] = ''
			})
